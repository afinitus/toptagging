...
if __name__ == "__main__":
    ...
    if flags.sample:    
        ...
        for process in processes:
            ...
            gluon_parts, gluon_jets, init_noise_parts, init_noise_jets, mask =model_gluon.generate(10)
            nll_qcd[process] = evaluate(model_gluon,gluon_parts,gluon_jets,mask,flags.nidx)
            #nll_top[process] = evaluate(model_top,particles,jets,mask,flags.nidx)
            ll_gluon_parts, final_noise_parts = model_gluon.Likelihood(gluon_parts.numpy(), model_gluon.ema_part, gluon_jets.numpy(),mask.astype(np.float32))
            ll_gluon_jets, final_noise_jets = model_gluon.Likelihood(gluon_jets.numpy(), model_gluon.ema_jet)
     
        # print(init_noise_parts)
        # print(final_noise_parts)
        # print(init_noise_jets)
        # print(final_noise_jets)
        # #in_p_flat = init_noise_parts.flatten()
        # #final_noise_parts.flatten()
        # #ADDED THIS CHANGE:
        # in_p_flat = tf.reshape(init_noise_parts, [-1])
        # tf.reshape(final_noise_parts, [-1])
...
#         fig,gs = utils.SetFig("True positive rate","Fake Rate")
    ...
    if flags.sup:
                # Density ratio for gluon tagging
                llr_gluon = -nll_qcd['gluon_tagging']['ll_part'] - nll_qcd['gluon_tagging']['ll_jet']
                # Assuming there are true labels for gluon tagging to calculate ROC
                labels_gluon = np.zeros(llr_gluon.shape[0])  # or your actual labels for gluon tagging
                fpr_gluon, tpr_gluon, _ = roc_curve(labels_gluon, llr_gluon, pos_label=1)
                print("Gluon tagging AUC: {}".format(auc(fpr_gluon, tpr_gluon)))

                # Plot ROC curve for gluon tagging
                plt.plot(tpr_gluon, fpr_gluon, label="Gluon Tagging", color='black', linestyle='--')
